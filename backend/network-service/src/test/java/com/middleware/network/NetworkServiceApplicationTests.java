...
class NetworkServiceApplicationTests {
    ...
    @Test
    @DisplayName("TC-NETWORK-001 默认 profile 独立加载网络岗位且关闭 Nacos")
    void defaultProfileLoadsWithNacosDisabled() {
        assertThat(environment.getProperty("server.port", Integer.class)).isEqualTo(8088);
        assertThat(environment.getProperty("spring.application.name")).isEqualTo("network-service");
    ...
    @Test
    @DisplayName("TC-NETWORK-002 GET /health 无需认证")

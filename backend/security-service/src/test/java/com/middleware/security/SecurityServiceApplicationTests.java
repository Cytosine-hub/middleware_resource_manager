...
class SecurityServiceApplicationTests {
    ...
    @Test
    @DisplayName("TC-SECURITY-001 默认 profile 独立加载网络安全岗位且关闭 Nacos")
    void defaultProfileLoadsWithNacosDisabled() {
        assertThat(environment.getProperty("server.port", Integer.class)).isEqualTo(8089);
        assertThat(environment.getProperty("spring.application.name")).isEqualTo("security-service");
    ...
    @Test
    @DisplayName("TC-SECURITY-002 GET /health 无需认证")

...
**LangChain4j 升级到 1.0.0 稳定版**

- 从 1.0.0-beta5 升级到 1.0.0 稳定版
- 主要 API 变更：`ChatResponse.aiMessage()` → `ChatResponse.content()`，`EmbeddingModel.embed()` 返回类型调整
- 升级时机：在阶段一实施时一并完成，作为改造的前置任务
- 升级范围：`TroubleshootAgent`、`EmbeddingService`、`AiConfig` 的适配
...
### 阶段一：数据模型 + Ingest 基础（2-3 周）

**目标：** 跑通"文档 → LLM 编译 → Wiki 页面"主线

- [ ] LangChain4j 升级 1.0.0-beta5 → 1.0.0 稳定版，适配 API 变更
- [ ] 引入 JGraphT、Commonmark 依赖
- [ ] 新建 `wiki_pages`、`wiki_links`、`wiki_sources`、`wiki_ingest_log` 表
- [ ] 实现 `WikiPageRepository`、`WikiSourceRepository`（JdbcTemplate）
- [ ] 实现 `IngestAgent` 两步编译流程
- [ ] 实现 `LinkResolver`（解析 `[[页面名]]` 建立 wiki_links）
- [ ] 实现导出包生成（JSON + Markdown）
- [ ] 实现导入程序（解析导出包写入内网数据库）
- [ ] 编译 Prompt 模板设计与调优
- [ ] 向量化写入（可选，仅当 Milvus 可用时，与现有 knowledge_chunks 分开）

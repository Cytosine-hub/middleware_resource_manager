...
- **阶段 1 — 引入网关 + Nacos（已落地）**：加 `api-gateway`；默认 profile 静态路由到 `app:8081` 以兼容无 Nacos 环境，`cloud` profile 下 app/Gateway 注册到 Nacos、Gateway 通过 `lb://infra-portal-app` 动态路由；前端 `VITE_*_API_BASE_URL` 指向网关。
